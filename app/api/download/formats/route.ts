...
import { buildYtdlpArgv, parseFormats } from "@/lib/download/argv-builder";
...
export async function GET(req: NextRequest) {
  return handleRoute(async () => {
    ...
    const argv = buildYtdlpArgv(safe, { listFormats: true });
    const result = await runYtdlpOrThrow(argv, { timeoutSec: 60 });
    return json({ formats: parseFormats(result.stdout) });
  });
}
